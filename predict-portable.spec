...
# 强制收集 Windows 下 the MSVC++ 核心运行时 DLLs，解决干净 Windows 环境下 DLL 初始化或找不到的崩溃问题
vc_binaries = []
...
app_name = 'Suno Checker' if is_mac else 'suno-checker-portable'
...
# 单文件打包配置（EXE 节点内包含 a.binaries、a.zipfiles、a.datas，无 COLLECT）
exe = EXE(
    pyz,
    a.scripts,
    a.binaries,
